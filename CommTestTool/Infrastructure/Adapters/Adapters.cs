    ...
    public async Task WriteAsync(string nodeId, IReadOnlyDictionary<string,string> payload, CancellationToken ct = default)
    {
        ...
        else
        {
            ...
            for (int i = 0; i < nodeIds.Length; i++)
            {
                ...
                var raw  = payload.GetValueOrDefault(nodeIds[i], null)
                        ?? payload.GetValueOrDefault(i.ToString(), "");
                var type = payload.GetValueOrDefault($"type_{i}", payload.GetValueOrDefault("type", "string"));
...
public sealed class MqttAdapter(ConnectionModel conn) : IProtocolAdapter
{
    ...
    public async Task ConnectAsync(CancellationToken ct = default)
    {
        ...
        var opts = new MqttClientOptionsBuilder()
            ...
            .WithCleanStart(true)   // v5: WithCleanSession() → WithCleanStart()
            .Build();
        _client.ApplicationMessageReceivedAsync += e =>
        {
            // v5: PayloadSegment は引き続き使用可能
