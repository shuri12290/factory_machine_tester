...
public partial class ExecutionViewModel : ViewModelBase
{
    ...
    private async Task RunCommand()
    {
        ...
        var progress = new Progress<StepResult>(sr =>
        {
            ...
            if (item != null) { item.Status = sr.Status; item.ErrorMessage = sr.ErrorMessage; }
        });
...
public partial class ParamValueItem : ObservableObject
{
    ...
    public bool IsTextType     => !IsBoolType && !IsBitType && !IsJsonType && !IsDateTimeType;
    ...
    public string TypeHint => Parameter.Type switch
    {
        ...
        "word"     => "0〜65535",
        "dword"    => "0〜4294967295",
        "long"     => "64bit整数 例: 1000000000000",
        "uint"     => "符号なし整数 0〜4294967295",
        "string"   => "テキスト",
        _          => ""
    };

    // boolean → CheckBox バインディング
    ...
    // bit → RadioButton バインディング
    public bool IsBitZero
    {
        get => Value != "1";
        set { if (value) Value = "0"; }
    }
    public bool IsBitOne
    {
        get => Value == "1";
        set { if (value) Value = "1"; }
    }
