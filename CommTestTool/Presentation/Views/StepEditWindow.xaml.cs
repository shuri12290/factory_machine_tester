...
public class PayloadRow : System.ComponentModel.INotifyPropertyChanged
{
    private string _key = "", _paramName = "";
    public string Key       { get => _key;       set { _key       = value; Notify(nameof(Key));       } }
    public string ParamName { get => _paramName; set { _paramName = value; Notify(nameof(ParamName)); } }
    public List<string> ParamOptions { get; set; } = new();
    public event System.ComponentModel.PropertyChangedEventHandler? PropertyChanged;
    private void Notify(string n) => PropertyChanged?.Invoke(this, new(n));
}
public class SlmpWriteRow : System.ComponentModel.INotifyPropertyChanged
{
    private string _address = "", _paramName = "";
    public string Address   { get => _address;   set { _address   = value; Notify(nameof(Address));   } }
    public string ParamName { get => _paramName; set { _paramName = value; Notify(nameof(ParamName)); } }
    public List<string> ParamOptions { get; set; } = new();
    public event System.ComponentModel.PropertyChangedEventHandler? PropertyChanged;
    private void Notify(string n) => PropertyChanged?.Invoke(this, new(n));
}
public class MethodInputRow : System.ComponentModel.INotifyPropertyChanged
{
    private string _typeName = "string", _paramName = "", _label = "";
    public string TypeName    { get => _typeName;  set { _typeName  = value; Notify(nameof(TypeName));    } }
    public string ParamName   { get => _paramName; set { _paramName = value; Notify(nameof(ParamName));   } }
    public string Label       { get => _label;     set { _label     = value; Notify(nameof(Label));       } }
    private int _displayIndex = 1;
    public int  DisplayIndex { get => _displayIndex; set { _displayIndex = value; Notify(nameof(DisplayIndex)); } }
    public List<string> ParamOptions { get; set; } = new();
    public event System.ComponentModel.PropertyChangedEventHandler? PropertyChanged;
    private void Notify(string n) => PropertyChanged?.Invoke(this, new(n));
}

/// <summary>OPC-UA メソッド出力引数行（自動採番インデックス付き）</summary>
public class MethodOutputRow : System.ComponentModel.INotifyPropertyChanged
{
    private string _as = "", _label = "";
    private int    _index;
    public int    Index        { get => _index; set { _index = value; Notify(nameof(Index)); Notify(nameof(DisplayIndex)); } }
    public int    DisplayIndex => _index + 1;   // 1始まり表示用
    public string As    { get => _as;    set { _as    = value; Notify(nameof(As));    } }
    public string Label { get => _label; set { _label = value; Notify(nameof(Label)); } }
    public List<string> ParamOptions { get; set; } = new();
    public event System.ComponentModel.PropertyChangedEventHandler? PropertyChanged;
...
public partial class StepEditWindow : Window
{
    ...
    private readonly ObservableCollection<PayloadRow>  _payloadRows  = [];   // TCP/MQTT send
    private readonly ObservableCollection<SlmpWriteRow>_slmpWriteRows= [];   // SLMP write（複数）
    private readonly ObservableCollection<OpcNodeRow>  _slmpReadRows = [];   // SLMP read（複数）
    ...
    private readonly ObservableCollection<MethodInputRow>  _methodInputRows  = [];  // opcua_method入力引数
    private readonly ObservableCollection<MethodOutputRow> _methodOutputRows = [];  // opcua_method出力引数
    ...
    private static readonly Dictionary<string, string[]> ProtoActions = new()
    {
        ["tcp"]        = ["send", "receive"],
        ["mqtt"]       = ["send", "receive"],
        ["opcua"]      = ["write", "read", "poll", "opcua_method"],
    ...
    private static readonly Dictionary<string, string> ActionHints = new()
    {
        ...
        ["opcua_method"] = "OPC-UA サーバーのメソッドを呼び出します。入力引数を渡し、出力引数を変数に保存できます。",
    };
    ...
    public StepEditWindow(StepModel step, List<ConnectionModel> connections, List<string> paramNames)
    {
        ...
        MethodInputList.ItemsSource  = _methodInputRows;
        MethodOutputList.ItemsSource = _methodOutputRows;
        PayloadList.ItemsSource   = _payloadRows;
        SlmpWriteList.ItemsSource = _slmpWriteRows;
        SlmpReadList.ItemsSource  = _slmpReadRows;
        ...
        // SLMP write は _slmpWriteRows で管理（SlmpWriteAddrBox/SlmpWriteValueBox は廃止）
        ...
        // SLMP read は _slmpReadRows で管理（SlmpReadAddrBox は廃止）
        ...
        // TCP/MQTT send payload をリストに復元
        _payloadRows.Clear();
        if (step.Payload != null)
            foreach (var kv in step.Payload)
                _payloadRows.Add(new PayloadRow { Key = kv.Key, ParamName = kv.Value, ParamOptions = _paramNames });
        if (!_payloadRows.Any())
            _payloadRows.Add(new PayloadRow { ParamOptions = _paramNames });

        // SLMP write を複数行リストに復元
        _slmpWriteRows.Clear();
        if (step.Action == "write" && step.Address != null)
        {
            _slmpWriteRows.Add(new SlmpWriteRow { Address = step.Address, ParamName = step.Parameter ?? "", ParamOptions = _paramNames });
        }
        if (!_slmpWriteRows.Any())
            _slmpWriteRows.Add(new SlmpWriteRow { ParamOptions = _paramNames });

        // SLMP read を複数行リストに復元
        _slmpReadRows.Clear();
        if (step.Action == "read" && step.Address != null)
            _slmpReadRows.Add(new OpcNodeRow { NodeId = step.Address });
        if (!_slmpReadRows.Any())
            _slmpReadRows.Add(new OpcNodeRow());

        // opcua_method 復元
        MethodObjectBox.Text = (step.Action == "opcua_method") ? step.NodeId ?? "" : "";
        MethodIdBox.Text     = (step.Action == "opcua_method") ? step.MethodId ?? "" : "";
        _methodInputRows.Clear();
        if (step.Action == "opcua_method" && step.Payload != null)
        {
            int ri = 0;
            foreach (var kv in step.Payload)
            {
                // キー形式: "0:string" (idx:型名) または旧形式 "string"
                var typeName = kv.Key.Contains(':') ? kv.Key.Split(':')[1] : kv.Key;
                _methodInputRows.Add(new MethodInputRow
                    { TypeName = typeName, ParamName = kv.Value, ParamOptions = _paramNames, DisplayIndex = ++ri });
            }
        }
        if (step.Action == "opcua_method" && !_methodInputRows.Any())
            _methodInputRows.Add(new MethodInputRow { ParamOptions = _paramNames });

        _methodOutputRows.Clear();
        if (step.Action == "opcua_method")
        {
            int idx = 0;
            foreach (var c in step.Capture ?? [])
                _methodOutputRows.Add(new MethodOutputRow
                    { Index = idx++, As = c.As, Label = "", ParamOptions = _paramNames });
        }
    ...
    // SlmpWriteParamCombo_Changed は新UI（SlmpWriteList）で不要のため削除
    ...
    private void UpdatePanels(ConnectionModel? conn, string? action)
    {
        foreach (var p in new FrameworkElement[]
            { WaitPanel, PollPanel, OpcWritePanel, SlmpWritePanel, TcpSendPanel,
              ...
              FocasReadPanel, FocasWritePanel, OpcUaMethodPanel })   // ← 追加
        ...
        switch (action)
        {
            ...
            case "poll":
                ...
                break;
            case "opcua_method":
                OpcUaMethodPanel.Visibility = Visibility.Visible;
                break;
        }
    ...
    private void Save_Click(object sender, RoutedEventArgs e)
    {
        ...
        Result = action switch
        {
            ...
            "opcua_method" => new StepModel("opcua_method", DescBox.Text.Trim(),
                ConnectionId: conn?.Id,
                NodeId:    MethodObjectBox.Text.Trim().NullIfEmpty(),
                MethodId:  MethodIdBox.Text.Trim().NullIfEmpty(),
                Payload:   _methodInputRows.Where(r => !string.IsNullOrWhiteSpace(r.ParamName))
                               .Select((r, idx) => (idx, r))
                               .ToDictionary(t => $"{t.idx}:{t.r.TypeName}", t => t.r.ParamName.Trim()) is var md && md.Any() ? md : null,
                Capture:   _methodOutputRows.Where(r => !string.IsNullOrWhiteSpace(r.As))
                               .Select(r => new CaptureModel(r.Index.ToString(), r.As.Trim())).ToList()),
            ...
            _ => new StepModel(action, DescBox.Text.Trim(),
                ...
                Address: conn?.Protocol == "slmp"
                    ? (_slmpReadRows.Count(r => !string.IsNullOrWhiteSpace(r.NodeId)) == 1
                        ? _slmpReadRows.First(r => !string.IsNullOrWhiteSpace(r.NodeId)).NodeId.Trim()
                        : null)
                    : null,
                Nodes: conn?.Protocol == "slmp" && _slmpReadRows.Count(r => !string.IsNullOrWhiteSpace(r.NodeId)) > 1
                    ? _slmpReadRows.Where(r => !string.IsNullOrWhiteSpace(r.NodeId))
                                   .Select(r => new NodeEntry(r.NodeId.Trim())).ToList()
                    : conn?.Protocol == "opcua" && _opcReadNodes.Count(r => !string.IsNullOrWhiteSpace(r.NodeId)) > 1
    ...
    private List<string> Validate(string action, ConnectionModel? conn)
    {
        ...
        if (action == "opcua_method")
        {
            if (string.IsNullOrWhiteSpace(MethodObjectBox.Text)) e.Add("・オブジェクトNodeIDを入力してください。");
            if (string.IsNullOrWhiteSpace(MethodIdBox.Text))     e.Add("・メソッドNodeIDを入力してください。");
            return e;
        }
        if (action == "poll")
        {
        ...
        if (action == "write")
        {
            ...
            if (conn.Protocol == "slmp" && _slmpWriteRows.All(r => string.IsNullOrWhiteSpace(r.Address)))
                e.Add("・アドレスを入力してください。");
        }
        if (action is "read")
        {
            ...
            if (conn.Protocol == "slmp" && _slmpReadRows.All(r => string.IsNullOrWhiteSpace(r.NodeId)))
                e.Add("・アドレスを入力してください。");
        }
        return e;
    }
    ...
    private void BuildParamCombos()
    {
        foreach (var combo in new[] { DurationParamCombo, TimeoutParamCombo,
            PollIntervalParamCombo, PollReadTimeoutParamCombo, PollTimeoutParamCombo })
        {
            combo.Items.Clear();
    ...
    private IReadOnlyDictionary<string,string>? BuildPayload()
    {
        var dict = _payloadRows
            .Where(r => !string.IsNullOrWhiteSpace(r.Key) && !string.IsNullOrWhiteSpace(r.ParamName))
            .ToDictionary(r => r.Key.Trim(), r => r.ParamName.Trim());
        return dict.Any() ? dict : null;
    }

    private void PayloadAdd_Click(object s, RoutedEventArgs e)      => _payloadRows.Add(new PayloadRow { ParamOptions = _paramNames });
    private void PayloadRemove_Click(object s, RoutedEventArgs e)   { if ((s as Button)?.Tag is PayloadRow r && _payloadRows.Count > 1) _payloadRows.Remove(r); }
    private void PayloadParamCombo_Changed(object sender, SelectionChangedEventArgs e)
    {
        if (sender is ComboBox c && c.SelectedItem is string p && c.Tag is PayloadRow r && !string.IsNullOrEmpty(p))
            r.ParamName = p;
    }
    private void SlmpWriteAdd_Click(object s, RoutedEventArgs e)    => _slmpWriteRows.Add(new SlmpWriteRow { ParamOptions = _paramNames });
    private void SlmpWriteRemove_Click(object s, RoutedEventArgs e) { if ((s as Button)?.Tag is SlmpWriteRow r && _slmpWriteRows.Count > 1) _slmpWriteRows.Remove(r); }
    private void SlmpWriteParamRow_Changed(object sender, SelectionChangedEventArgs e)
    {
        if (sender is ComboBox c && c.SelectedItem is string p && c.Tag is SlmpWriteRow r && !string.IsNullOrEmpty(p))
            r.ParamName = p;
    }
    private void SlmpReadAdd_Click(object s, RoutedEventArgs e)     => _slmpReadRows.Add(new OpcNodeRow());
    private void SlmpReadRemove_Click(object s, RoutedEventArgs e)  { if ((s as Button)?.Tag is OpcNodeRow r && _slmpReadRows.Count > 1) _slmpReadRows.Remove(r); }
    ...
    // ── OPC-UA Method 入力引数・出力引数操作 ─────────────────────────────────
    private void MethodInputAdd_Click(object s, RoutedEventArgs e)
        => _methodInputRows.Add(new MethodInputRow { ParamOptions = _paramNames, DisplayIndex = _methodInputRows.Count + 1 });
    private void MethodInputRemove_Click(object s, RoutedEventArgs e)
    {
        if ((s as Button)?.Tag is MethodInputRow r)
        {
            _methodInputRows.Remove(r);
            // DisplayIndex を振り直す（1始まり）
            for (int i = 0; i < _methodInputRows.Count; i++)
                _methodInputRows[i].DisplayIndex = i + 1;
        }
    }
    private void MethodInputParamCombo_Changed(object sender, SelectionChangedEventArgs e)
    {
        if (sender is ComboBox c && c.SelectedItem is string p && c.Tag is MethodInputRow r && !string.IsNullOrEmpty(p))
            r.ParamName = p;
    }
    private void MethodOutputAdd_Click(object s, RoutedEventArgs e)
    {
        _methodOutputRows.Add(new MethodOutputRow { Index = _methodOutputRows.Count, ParamOptions = _paramNames });
    }
    private void MethodOutputRemove_Click(object s, RoutedEventArgs e)
    {
        if ((s as Button)?.Tag is MethodOutputRow r)
        {
            _methodOutputRows.Remove(r);
            // インデックスを振り直す
            for (int i = 0; i < _methodOutputRows.Count; i++) _methodOutputRows[i].Index = i;
        }
    }
    private void MethodOutputParamCombo_Changed(object sender, SelectionChangedEventArgs e)
    {
        if (sender is ComboBox c && c.SelectedItem is string p && c.Tag is MethodOutputRow r && !string.IsNullOrEmpty(p))
            r.As = p;
    }
    ...
    private StepModel BuildWriteStep(ConnectionModel? conn, string desc)
    {
        ...
        {
            var validSlmp = _slmpWriteRows.Where(r => !string.IsNullOrWhiteSpace(r.Address)).ToList();
            if (validSlmp.Count > 1)
            {
                // 複数アドレス → Nodes リストで保存（NodeEntry の NodeId=Address, Parameter=paramName）
                return new StepModel("write", desc,
                    ConnectionId: conn.Id,
                    Nodes: validSlmp.Select(r => new NodeEntry(r.Address.Trim(), r.ParamName.Trim().NullIfEmpty())).ToList());
            }
            var row = validSlmp.FirstOrDefault() ?? _slmpWriteRows.FirstOrDefault();
            return new StepModel("write", desc,
                ConnectionId: conn.Id,
